...
const RecruiterDashboard = () => {
  ...
  const fetchFeedbackCount = async () => {
    try {
      const response = await jobAPI.getRecruiterFeedbackCount();
      setFeedbackCount(response.data.newCount || 0);
    } catch (err) {
    ...
    <div className="dashboard-container">
      ...
      <div className="dashboard-content">
        <div className="sidebar">
          ...
          <button
            className="nav-btn"
            onClick={() => navigate('/recruiter/feedbacks')}
            style={{ position: 'relative', paddingRight: feedbackCount > 0 ? '35px' : '15px' }}
          >
