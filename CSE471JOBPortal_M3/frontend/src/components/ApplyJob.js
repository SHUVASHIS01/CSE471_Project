...
import { jobAPI, draftAPI, resumeAPI } from '../api';
...
const ApplyJob = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Prepare application data with FormData for file upload support
      const applicationFormData = new FormData();
      
      if (resumeSource === 'existing' && selectedResumeId) {
        // Using an uploaded resume
        applicationFormData.append('resumeId', selectedResumeId);
      } else if (resumeSource === 'upload' && resumeFile) {
        // Uploading a new file
        applicationFormData.append('resumeFile', resumeFile);
      }
      
      applicationFormData.append('coverLetter', formData.coverLetter);
      // Send skills as comma-separated string (backend handles both array and string)
      applicationFormData.append('skills', formData.skills);
      applicationFormData.append('experience', formData.experience);
      
      await jobAPI.applyForJob(jobId, applicationFormData);
        ...
        <div className="application-form-card">
          ...
          <form onSubmit={handleSubmit} className="application-form">
            ...
            <div className="form-group">
              ...
              <div className="resume-source-selector" style={{ marginBottom: '16px' }}>
                <label style={{ display: 'flex', gap: '12px', alignItems: 'center', marginBottom: '12px' }}>
                  <input
                    type="radio"
                    name="resumeSource"
                    value="existing"
                ...
                <label style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                  ...
                  />
                  <span>Upload new resume from local storage</span>
                </label>
              </div>

              {/* Existing Resumes Dropdown */}
              {resumeSource === 'existing' && (
                <div className="resume-selector" style={{ marginBottom: '16px' }}>
                  {loadingResumes ? (
                    <div style={{ padding: '12px', color: '#6b7280' }}>Loading resumes...</div>
                  ) : uploadedResumes.length === 0 ? (
                    <div style={{ padding: '12px', background: '#fef3c7', border: '1px solid #fbbf24', borderRadius: '8px', color: '#92400e' }}>
                      No resumes uploaded yet. Please upload a resume from your profile or select "Upload new resume" option.
                    </div>
                  ) : (
                    <>
                      <select
                        ...
                        required={resumeSource === 'existing'}
                      >
                        <option value="">-- Select a resume --</option>
                        {uploadedResumes.map(resume => (
                          <option key={resume._id} value={resume._id}>
                            {resume.title} {resume.isDefault ? '⭐ (Default)' : ''} - {new Date(resume.createdAt).toLocaleDateString()}
                            {resume.lastUsedAt ? ` - Last used: ${new Date(resume.lastUsedAt).toLocaleDateString()}` : ''}
                          </option>
                        ))}
                      </select>
                      {selectedResumeId && (
                        <div style={{ padding: '8px 12px', background: '#f0fdf4', border: '1px solid #10b981', borderRadius: '6px', fontSize: '13px', color: '#065f46' }}>
                          ✓ Resume selected. You can also set this as default for future applications from your profile.
                        </div>
                      )}
                    </>
                  )}
