...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      const profileKeywordsInput = req.body.profileKeywords;
      ...
      // Add profile keywords support to main profile update
      if (typeof profileKeywordsInput !== 'undefined') {
        const keywordsArray = Array.isArray(profileKeywordsInput)
          ? profileKeywordsInput
          : profileKeywordsInput.split(',').map(k => k.trim()).filter(Boolean);
        ...
        update.profileKeywords = sanitized;
      }
